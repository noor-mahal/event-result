...
// state.groups / state.students / state.competitions / state.results / state.gallery
// are now loaded FROM SUPABASE (see loadAll()) instead of hardcoded constants.
let state={page:"home",selectedId:null,mobile:false,groups:[],students:[],competitions:[],results:{},gallery:[],signed:false,loading:true};
let query="",filter="all",adminTab="dashboard",loginView="signin",loginError="",forgotError="",forgotEmail="",newPass="",newPass2="",justPublished=null,adminDraft={};
...
function totals(){return state.groups.map(g=>({...g,total:Object.values(state.results).reduce((sum,r)=>sum+(r.published?["first","second","third"].reduce((x,p)=>x+(r[p]||[]).reduce((z,s)=>z+(s?.groupId===g.id?PLACE_POINTS[p]:0),0),0):0),0)}))}
...
async function initAuth(){
  ...
  supabase.auth.onAuthStateChange((event,session)=>{
    if(event==="PASSWORD_RECOVERY"){loginView="forgot-newpass";render();return}
    state.signed=!!session;render();
  });
}
...
// NAV / RENDER
// ============================================================
function go(p,id=null){state.page=p;if(id)state.selectedId=id;state.mobile=false;render();scrollTo(0,0)}
...
function resultPage(){
  let c=state.competitions.find(x=>x.id===state.selectedId),r=state.results[state.selectedId],a=!!r?.published;
...
async function savePoster(cid){let c=state.competitions.find(x=>x.id===cid),canvas=document.getElementById("poster");if(!canvas)return;drawPoster(cid);let data=canvas.toDataURL("image/png");let a=document.createElement("a");a.href=data;a.download=`${c.name.replace(/\s+/g,"-")}-${posterTheme}-poster.png`;a.click();
  const {error}=await supabase.from("gallery").insert({competition_name:c.name,style:posterTheme,image_data:data});
  if(error){alert("Could not save to Gallery: "+error.message);return}
...
function totalPage(){let ts=totals().sort((a,b)=>b.total-a.total);return `<div class="container narrow"><div class="pagehead"><div><h1 class="display">🏅 Total Result</h1><p class="muted">Overall standings across every announced competition.</p></div>${live()}</div><div class="grid grid3" style="margin-top:30px">${ts.slice(0,3).map((g,i)=>`<div class="place ${["first","second","third"][i]}"><span class="medal">${["🥇","🥈","🥉"][i]}</span><span class="label">${["1st Place","2nd Place","3rd Place"][i]}</span><span class="name">${g.name}</span><span>${g.total} Marks</span></div>`).join("")}</div><p class="center muted" style="font-size:12px;margin-top:20px">Scoring: 1st = 20 marks · 2nd = 10 marks · 3rd = 7 marks.</p>${lookup()}</div>`}
...
async function signIn(){let e=document.getElementById("lemail").value.trim(),p=document.getElementById("lpass").value;const {error}=await supabase.auth.signInWithPassword({email:e,password:p});if(error){loginError="Incorrect email or password.";render();return}loginError="";state.signed=true;render()}
async function sendReset(){let e=document.getElementById("femail").value.trim();const {error}=await supabase.auth.resetPasswordForEmail(e,{redirectTo:window.location.origin});if(error){forgotError=error.message;render();return}forgotError="";alert("Reset link sent — check the admin inbox.");loginView="signin";render()}
async function saveNewPass(){let a=document.getElementById("np1").value,b=document.getElementById("np2").value;if(a.length<6){forgotError="Password must be at least 6 characters.";render();return}if(a!==b){forgotError="Passwords don't match.";render();return}const {error}=await supabase.auth.updateUser({password:a});if(error){forgotError=error.message;render();return}forgotError="";loginView="signin";render()}
function signOut(){supabase.auth.signOut();state.signed=false;render()}

// ============================================================
// ADMIN PANEL (all writes go straight to Supabase)
// ============================================================
...
async function addComp(){let n=document.getElementById("cn").value.trim(),c=document.getElementById("cc").value;if(!n)return;const {error}=await supabase.from("competitions").insert({id:newId(),name:n,category:c});if(error)alert(error.message)}
async function editComp(id){let c=state.competitions.find(x=>x.id===id),n=prompt("Competition name:",c.name);if(n===null)return;let cat=prompt("Category:",c.category);if(!n.trim())return;const patch={name:n.trim()};if(CATEGORIES.includes(cat))patch.category=cat;const {error}=await supabase.from("competitions").update(patch).eq("id",id);if(error)alert(error.message)}
async function deleteComp(id){if(!confirm("Delete this competition? This also removes its result."))return;const {error}=await supabase.from("competitions").delete().eq("id",id);if(error)alert(error.message)}
function manageStudents(){return `<div class="grid grid2" style="margin-top:25px">${state.groups.map(g=>{let ss=state.students.filter(s=>s.groupId===g.id);return `<div class="card"><h2 class="display">${g.name}</h2><p class="muted">Students in this group.</p><ul class="list">${ss.map(s=>`<li><span><b>${esc(s.name)}</b><small class="muted"> · ID: ${esc(s.studentId)}</small></span><button class="smallbtn danger" onclick="delStudent('${s.id}')">Delete</button></li>`).join("")||"<li>No students yet.</li>"}</ul><div class="field"><label>Student ID</label><input id="sid-${g.id}" class="input" placeholder="e.g. K01"></div><div class="field" style="margin-top:8px"><label>Student Name</label><input id="sn-${g.id}" class="input"></div><button class="btn green" style="margin-top:10px" onclick="addStudent('${g.id}')">Add Student</button></div>`}).join("")}</div>`}
async function addStudent(gid){let id=document.getElementById("sid-"+gid).value.trim(),n=document.getElementById("sn-"+gid).value.trim();if(!id||!n)return;const {error}=await supabase.from("students").insert({id:newId(),group_id:gid,student_code:id,name:n});if(error)alert(error.message)}
async function delStudent(id){if(!confirm("Remove this student from the roster?"))return;const {error}=await supabase.from("students").delete().eq("id",id);if(error)alert(error.message)}
...
function draftResult(id){let r=state.results[id]||{};if(!adminDraft[id])adminDraft[id]={first:(r.first||[]).map(x=>({...x})),second:(r.second||[]).map(x=>({...x})),third:(r.third||[]).map(x=>({...x}))};["first","second","third"].forEach(p=>{if(!adminDraft[id][p].length)adminDraft[id][p]=[{groupId:"g1",studentId:state.students.find(s=>s.groupId==="g1")?.studentId||""}]});return adminDraft[id]}
function selectedAdminComp(id){let c=state.competitions.find(x=>x.id===id);if(!c){document.getElementById("resultForm").innerHTML="";return}let d=draftResult(id),html=`<div class="card" style="margin-top:20px"><p class="muted">Status: ${state.results[id]?.published?"🟢 Announced":"🟡 Not Announced"}</p>`;[["first","🥇 1st Place (20 marks)"],["second","🥈 2nd Place (10 marks)"],["third","🥉 3rd Place (7 marks)"]].forEach(([p,label])=>{html+=`<div class="card" style="margin-top:12px;padding:12px"><b>${label}</b>`;d[p].forEach((x,i)=>{let g=x.groupId||"g1";html+=`<div class="result-entry" style="display:grid;grid-template-columns:1fr 1fr auto;gap:8px;margin-top:8px;align-items:center"><select class="select" onchange="updateResultDraft('${id}','${p}',${i},'group',this.value)">${state.groups.map(z=>`<option value="${z.id}" ${z.id===g?"selected":""}>${z.name}</option>`).join("")}</select><select class="select" onchange="updateResultDraft('${id}','${p}',${i},'student',this.value)">${state.students.filter(z=>z.groupId===g).map(z=>`<option value="${z.studentId}" ${z.studentId===x.studentId?"selected":""}>${esc(z.name)} (ID: ${esc(z.studentId)})</option>`).join("")}</select><button class="smallbtn danger" onclick="removeResultStudent('${id}','${p}',${i})" ${d[p].length===1?"disabled":""}>Remove</button></div>`});html+=`<button class="smallbtn" style="margin-top:10px" onclick="addResultStudent('${id}','${p}')">+ Add Student to ${label.replace(/\s*\(.*/, "")}</button></div>`});html+=`<div class="actions" style="justify-content:flex-start"><button class="btn gold" onclick="publishResult('${id}')">${state.results[id]?.published?"Update Result":"Publish Result"}</button>${state.results[id]?.published?`<button class="smallbtn" onclick="revokeResult('${id}')">Revoke Announcement</button>`:""}</div><p class="muted" style="font-size:11px">You can add multiple students to the same position. </p></div>`;document.getElementById("resultForm").innerHTML=html}
function updateResultDraft(id,p,i,type,value){let d=draftResult(id);if(type==="group"){d[p][i].groupId=value;d[p][i].studentId=state.students.find(s=>s.groupId===value)?.studentId||"";let row=event.target.closest(".result-entry"),sel=row?.querySelectorAll("select")[1];if(sel)sel.innerHTML=state.students.filter(s=>s.groupId===value).map(x=>`<option value="${x.studentId}">${esc(x.name)} (ID: ${esc(x.studentId)})</option>`).join("")}else d[p][i].studentId=value}
function addResultStudent(id,p){let d=draftResult(id),last=d[p][d[p].length-1]||{groupId:"g1"},g=last.groupId||"g1";d[p].push({groupId:g,studentId:state.students.find(s=>s.groupId===g)?.studentId||""});selectedAdminComp(id)}
function removeResultStudent(id,p,i){let d=draftResult(id);if(d[p].length>1){d[p].splice(i,1);selectedAdminComp(id)}}
async function publishResult(id){let d=draftResult(id),row={competition_id:id,published:true};["first","second","third"].forEach(p=>row[p]=d[p].filter(x=>x.studentId).map(x=>({groupId:x.groupId,studentId:x.studentId})));const {error}=await supabase.from("results").upsert(row);if(error){alert(error.message);return}justPublished=id;setTimeout(()=>{justPublished=null;render()},3000)}
async function revokeResult(id){const {error}=await supabase.from("results").update({published:false}).eq("competition_id",id);if(error)alert(error.message)}
...
function render(){
  if(state.loading){document.getElementById("app").innerHTML=`<div class="preview">Loading live data…</div>`;return}
